...
const DistributorLoginPage = () => {
  ...
  const steps = [
    { id: 'name', label: 'Distributor Name', placeholder: 'Enter your business name', icon: RiUserFill, field: 'name' },
  ...
  const handleNext = async () => {
    if (step === 0) {
      if (!validateName(formData.name)) return toast.error('Valid business name required.');
    }
    ...
    if (step === 1) {
      ...
      setLoading(true);
      const { success, message } = await requestOTP(formData.email, 'distributor');
      setLoading(false);
  ...
  const handleFinalLogin = async () => {
    ...
    setLoading(true);
    const { success, role, message } = await verifyOTP(formData.email, formData.otp, 'distributor', formData.name);
    setLoading(false);

    if (success && role === 'distributor') {
      toast.success(`Welcome DISTRIBUTOR, ${formData.name}`);
      navigate(`/distributor`);
    } else if (success) {
      toast.error('Unauthorized access. This portal is for Distributors only.');
    } else {
      ...
      <div className="w-full max-w-[450px] min-h-screen bg-white shadow-[0_0_50px_rgba(0,0,0,0.05)] flex flex-col relative overflow-hidden">
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-8 py-10 z-10 w-full">
          ...
          <div className="mt-4 text-center">
             <h1 className="text-xl font-black text-[#2D8F9C] tracking-tighter uppercase italic">DISTRIBUTOR LOGIN</h1>
             <p className="text-[#94A3B8] text-[8px] font-black tracking-[0.2em] uppercase opacity-70">SUPPLY CHAIN HUB</p>
          </div>
          ...
          <div className="w-full space-y-6">
            ...
            <div className="text-center pt-4">
              ...
              <button onClick={() => navigate('/distributor/signup')} className="text-[#E0128A] text-xs font-bold hover:underline">Apply Now</button>
            </div>
          </div>
        </div>
