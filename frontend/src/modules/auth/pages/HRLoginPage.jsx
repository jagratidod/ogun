...
import {
  RiUserSettingsLine, RiMailFill, RiShieldFlashFill,
...
const HRLoginPage = () => {
  ...
  const steps = [
    ...
    },
    {
      ...
      placeholder: 'hr.manager@ogun.in',
      icon: RiMailFill,
      type: 'email',
      field: 'email'
    },
  ...
  const handleNext = async () => {
    ...
    if (step === 1) {
      if (!formData.email) return toast.error('Corporate email required.');
      ...
      if (success) {
        ...
        toast.error(message || 'Failed to send access key');
      }
      return;
    }
  ...
  const handleFinalLogin = async () => {
    if (!formData.otp || formData.otp.length < 6) return toast.error('Enter valid 6-digit access key.');
    ...
    if (success && role === 'admin') {
      toast.success('HR Portal Access Granted');
      navigate('/hr');
    } else {
    ...
    <div className="min-h-screen bg-white flex flex-col font-sans overflow-hidden">
      ...
      <div className="flex flex-col items-center -mt-[25px] sm:-mt-[40px] px-6 relative z-30 transition-all duration-500">
        ...
        <div className="mt-5 text-center px-4">
          <h1 className="text-xl font-black text-brand-teal tracking-tighter uppercase leading-none italic">PEOPLE OPS</h1>
          <p className="text-gray-300 text-[9px] font-black tracking-[0.2em] mt-1.5 opacity-60 uppercase">HR COMMAND CENTER LOGIN</p>
        </div>
      </div>
